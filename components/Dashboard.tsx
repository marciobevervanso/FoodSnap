          ...
          <div className="max-w-5xl mx-auto">
             <header className="mb-8">
                ...
                <p className="text-gray-500 flex items-center gap-2">
                    ID: <span className="font-mono text-xs bg-gray-200 px-2 py-0.5 rounded text-gray-700">{user.public_id || 'Carregando...'}</span>
                    <span className="text-gray-300">•</span>
                    {t.dashboard.status}: <span className="font-semibold text-green-600 capitalize flex items-center gap-1"><CheckCircle2 size={12}/> Ativo</span>
                </p>
             </header>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <StatCard 
                    ...
                    value={planName}
                    sub={user.plan === 'pro' ? t.dashboard.activeSub : (user.plan === 'trial' ? t.dashboard.trialSub : t.dashboard.upgradeSub)}
                    icon={<CreditCard className="text-brand-500" />} 
                    highlight={user.plan === 'pro' || user.plan === 'trial'}
                />
             </div>
             ...
             <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-200 shadow-sm mb-10 flex flex-col md:flex-row gap-8 items-center relative overflow-hidden group">
                ...
                <div className="flex-1 space-y-6">
                    <div>
                        ...
                        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{t.dashboard.connectTitle} 🚀</h2>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="p-8 border-b border-gray-100">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-sm text-gray-500 uppercase font-bold tracking-wider mb-1">{t.dashboard.currentPlan}</p>
                                <h2 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
                                    {planName}
                                    {(user.plan === 'pro' || user.plan === 'trial') && <span className="text-xs bg-brand-100 text-brand-700 px-2 py-1 rounded-full border border-brand-200">Ativo</span>}
                                </h2>
                            ...
                            <div className={`h-12 w-12 rounded-full flex items-center justify-center ${user.plan === 'pro' ? 'bg-brand-100 text-brand-600' : 'bg-gray-100 text-gray-400'}`}>
                                <CreditCard size={24} />
                            </div>
                        </div>
                    </div>
...
const StatCard = ({ title, value, sub, icon, highlight }: any) => (
    <div className={`p-6 rounded-2xl border ${highlight ? 'bg-brand-50 border-brand-200' : 'bg-white border-gray-100'} shadow-sm flex items-start justify-between`}>
        <div>
            <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">{title}</p>
            <h4 className={`text-2xl font-extrabold ${highlight ? 'text-brand-900' : 'text-gray-900'}`}>{value}</h4>
            <p className="text-xs text-gray-400 mt-1">{sub}</p>
        </div>
        <div className={`p-2 rounded-lg ${highlight ? 'bg-brand-100' : 'bg-gray-50'}`}>
            {icon}
        </div>
    </div>
);
...
const HistoryCard = ({ item, fallback }: any) => (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow group cursor-pointer h-full flex flex-col">
        <div className="h-32 overflow-hidden relative bg-gray-100">
            ...
            {item.score > 0 && (
                <div className={`absolute bottom-2 left-2 text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm text-white ${item.score >= 80 ? 'bg-green-500' : 'bg-yellow-500'}`}>
                    Score {item.score}
                </div>
            )}
        </div>
