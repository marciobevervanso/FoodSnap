...
const RegistrationModal: React.FC<RegistrationModalProps> = ({
      ...
      if (isCompletingProfile) {
        ...
        if (upsertError) {
          ...
          if (
            ...
            throw new Error(
              'Este telefone já está cadastrado em outra conta. Use outro número ou faça login na conta original.'
            );
          }
  ...
  const title = isCompletingProfile
    ? t.auth.completeProfile
    : activeMode === 'login'
      ? t.auth.welcomeBack
      : t.auth.createAccount;

  const subtitle = isCompletingProfile
    ? t.auth.confirmPhone
    : activeMode === 'login'
      ? t.auth.accessPanel
      : t.auth.fillToAccess;
